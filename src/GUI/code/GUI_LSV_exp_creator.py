...
def load_folder(LSV):
    ...
    """
    LSV.experiment_file_path.setText(
        LSV.load_folder_name()
...
def LSV_window_writer(exp, config_data):
    ...
    exp.experiment_start_voltage.setText(
        str(config_data["linear_sweep_voltammetry"]["start_voltage"])
    )
    exp.experiment_end_voltage.setText(
        str(config_data["linear_sweep_voltammetry"]["end_voltage"])
    )
    exp.experiment_sweep_rate.setText(
        str(config_data["linear_sweep_voltammetry"]["sweep_rate"])
    )
...
def LSV_main(ui, config_data=None):
    ...
    LSV.select_output_filepath_button.clicked.connect(
        partial(load_folder, LSV)
    )
    LSV.advanced_parameters_button.clicked.connect(
        partial(load_APwindow, LSV, AP_params)
    )
    LSV.generate_preview_button.clicked.connect(
        partial(preview_LSV, LSV, grid)
    )
    LSV.save_experiment_file_button.clicked.connect(
        partial(write_to_file_LSV, ui, LSV, LSV_window, AP_params)
    )
    return LSV
