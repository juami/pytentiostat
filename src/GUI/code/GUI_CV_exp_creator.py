...
def load_folder(CV):
    ...
    CV : the Ui_CV object
    """
    CV.experiment_file_path.setText(
        CV.load_folder_name()
...
def CV_data(stp_num, cv_sv, cv_ftv, cv_stv, cv_sr):
    ...
    steps_list = np.concatenate(
        (first_steps_list, second_steps_list, third_steps_list)
    )

    first_times_steps = np.linspace(
        0, first_time_range, num=num1, endpoint=False
    )
    second_times_steps = np.linspace(
        first_time_range,
        first_time_range + second_time_range,
        num=num2,
        endpoint=False,
    )
    third_times_steps = np.linspace(
        first_time_range + second_time_range,
        first_time_range + second_time_range + third_time_range,
        num=num3,
    )
    times_step_list = np.concatenate(
        (first_times_steps, second_times_steps, third_times_steps)
    )
...
def CV_window_writer(exp, config_data):
    ...
    exp.experiment_start_voltage.setText(
        str(config_data["cyclic_voltammetry"]["start_voltage"])
    )
    exp.experiment_first_vertex_voltage.setText(
        str(config_data["cyclic_voltammetry"]["first_turnover_voltage"])
    )
    exp.experiment_second_vertex_voltage.setText(
        str(config_data["cyclic_voltammetry"]["second_turnover_voltage"])
    )
    exp.experiment_sweep_rate.setText(
        str(config_data["cyclic_voltammetry"]["sweep_rate"])
    )
    exp.experiment_cycle_number.setText(
        str(config_data["cyclic_voltammetry"]["number_of_cycles"])
    )
...
def CV_main(ui, config_data=None):
    ...
    CV.advanced_parameters_button.clicked.connect(
        partial(load_APwindow, CV, AP_params)
    )
    CV.generate_preview_button.clicked.connect(partial(preview_CV, CV, grid))
    CV.save_experiment_file_button.clicked.connect(
        partial(write_to_file_CV, ui, CV, CV_window, AP_params)
    )
