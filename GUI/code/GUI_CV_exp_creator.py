...
def preview_CV(exp, grid):
    ...
    cv_sv = float(exp.experiment_start_voltage.text())
    cv_ftv = float(exp.experiment_first_vertex_voltage.text())
    cv_stv = float(exp.experiment_second_vertex_voltage.text())
    cv_sr = float(exp.experiment_sweep_rate.text())
    cyc_num = int(exp.experiment_cycle_number.text())
