...
def calculate_time(filename):
    ...
    if 'CA' in filename:
        ...
        v1 = abs(config_data["cyclic_voltammetry"]["start_voltage"] - config_data["cyclic_voltammetry"][
            "first_turnover_voltage"])
        v2 = abs(config_data["cyclic_voltammetry"]["first_turnover_voltage"] - config_data["cyclic_voltammetry"][
            "second_turnover_voltage"])
        v3 = abs(config_data["cyclic_voltammetry"]["second_turnover_voltage"] - config_data["cyclic_voltammetry"][
            "start_voltage"])
...
def total_time(ui):
    """
    Calculate the experiment time of each file in the experiment queue and append them to a list.
...
def run_exp(ui,com,board_objects,ini_plot,grid,pr,tr):
    ...
    """
    if not com:
        warning('Please connect potentiostat!')   # make sure that the user connects potentiostat before measurement
        ui.start_abort_experiment_button.setChecked(False)
        return
    elif ui.experiment_queue.count() == 0 :
        warning('Please load a config file! ')                       # make sure that the config file is loaded before measurement
        ui.start_abort_experiment_button.setChecked(False)
        return
    ...
    for index in range(ui.experiment_queue.count()):
        ...
        config_path_name = os.path.join(data_out_path, data_out_name + "_" + exp_type + ".csv")
        ...
        ui.experiment_status_indicator.setChecked(True)
        if ui.stop_experiment_button.isChecked():
            break
        elif ui.start_abort_experiment_button.isChecked():
            times,voltages,currents = [],[],[]
            time, voltage, current = experiment(config_data, *board_objects, ini_plot, times, voltages,currents, pr, tr, total_exp_time,passed_exp_time)
            try:
                with open(config_path_name, "w") as csv_file:
    ...
    ui.start_abort_experiment_button.setChecked(False)
    ui.experiment_status_indicator.setChecked(False)

def run_exp_main(ui,com,board_objects,grid):
    """
    The main function of run experiment.
    Initialize ini_plot,pr,tr and pass it to the run_exp function
