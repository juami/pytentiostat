...
def load_folder(CA):
    """
    This function is connect with'Output Filepath' to load the folder window for the CA config file to be saved.
    ...
    CA.experiment_file_path.setText(CA.load_folder_name())  # CA.load_folder_name() returns folder path name
...
def config_writer_CA(exp_type, rt, stp_num, data_out_name, data_out_path,conv_fact,set_gain,set_offset,sr,ts,avg_num,
                     ca_v, ca_time):
    """
    ...
    config_data = {'general_parameters': {'experiment_type': exp_type,
                   ...
                   'chronoamperometry': {'voltage': ca_v,
                                         'time': ca_time},
...
def write_to_file_CA(ui,CA,CA_window,AP_params):
    ...
    filename_list = os.listdir(data_out_path)                 # list all the filename in the folder
...
def CA_window_writer(exp,config_data):
    ...
    exp.experiment_voltage.setText(str(config_data["chronoamperometry"]["voltage"]))
...
def CA_main(ui,config_data=None):
    ...
    CA,CA_window = ui.show_CAwindow()                      # Initialize the CA window - CA window pops up
    grid = QGridLayout()                         # Initialize the layout for preview graph
    CA.plot_area.setLayout(grid)
    AP_params = [4.798, 1.03, 0, 0.202, 0.003,9]  # Default AP_params: [conv_fact, set_gain, set_offset, sr, ts,avg_num]
    if config_data:
